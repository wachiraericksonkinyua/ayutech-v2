...
def main(page: ft.Page):
    ...
    def admin_delete(path, **kw):
        kw.setdefault("timeout", 5)
        return _check_session(httpx.delete(f"{API_BASE_URL}{path}", headers=admin_headers(), **kw))
...
Report generated by: {current_user.get('name')} ({current_user.get('phone') or 'Counter'})"""
    ...
    def build_admin_desk_tab():
        ...
        # ---- Staff / POS seller accounts ----
        staff_items = []

        staff_col = ft.Column(spacing=6)

        def fetch_staff():
            nonlocal staff_items
            try:
                res = admin_get(f"/admin/staff", timeout=8)
                if res.status_code == 200:
                    staff_items = res.json().get("staff", [])
            except Exception as e:
                print(f"Staff fetch error: {e}")

        def render_staff():
            staff_col.controls.clear()
            if not staff_items:
                staff_col.controls.append(
                    ft.Text("No staff accounts yet. Add one so a seller can use the POS.", size=11, color="#6B7280")
                )
                return
            for s in staff_items:
                ...
                staff_col.controls.append(
                    ft.Container(
                        ...
                        content=ft.Row([
                            ft.Column([
                                ft.Text(s.get("name") or "Staff", size=12, weight=ft.FontWeight.BOLD, color="#121212"),
                                ft.Text(f"{s.get('email') or '(no email)'} • {s.get('role', 'staff')}", size=10, color="#6B7280"),
                            ], expand=True, spacing=1),
                            ft.Container(
                                bgcolor="#E3F6EC" if is_active else "#F3F4F6",
                                padding=ft.padding.symmetric(horizontal=6, vertical=2),
                                border_radius=4,
                                content=ft.Text("ACTIVE" if is_active else "DISABLED", size=9,
                                                color="#0FA35C" if is_active else "#6B7280", weight=ft.FontWeight.BOLD),
                            ),
                            ft.IconButton(
                                ft.icons.BLOCK if is_active else ft.icons.CHECK_CIRCLE_OUTLINE,
                                icon_size=16, icon_color="#DC2626" if is_active else "#0FA35C",
                                tooltip="Disable" if is_active else "Enable",
                                on_click=lambda e, i=sid, a=is_active: toggle_staff_active(i, a),
                            ),
                            ft.IconButton(
                                ft.icons.DELETE_OUTLINE, icon_size=16, icon_color="#DC2626", tooltip="Delete",
                                on_click=lambda e, i=sid: delete_staff_account(i),
                            ),
                        ], spacing=4),
                    )
                )

        def toggle_staff_active(sid, is_active):
            try:
                admin_patch(f"/admin/staff/{sid}", json={"active": not is_active}, timeout=8)
                show_toast("Staff updated")
            except Exception as e:
                show_toast(f"Update failed: {e}", is_error=True)
            fetch_staff()
            render_staff()
            page.update()

        def delete_staff_account(sid):
            try:
                admin_delete(f"/admin/staff/{sid}", timeout=8)
                show_toast("Staff removed")
            except Exception as e:
                show_toast(f"Delete failed: {e}", is_error=True)
            fetch_staff()
            render_staff()
            page.update()

        def open_add_staff_dialog(e=None):
            name_f = ft.TextField(label="Full Name", width=280, text_size=13)
            email_f = ft.TextField(label="Email", width=280, text_size=13, keyboard_type=ft.KeyboardType.EMAIL)
            pw_f = ft.TextField(label="Password (min 6 chars)", width=280, text_size=13, password=True, can_reveal_password=True)
            phone_f = ft.TextField(label="Phone (optional)", width=280, text_size=13)
            role_f = ft.Dropdown(
                label="Role", width=280, value="staff", text_size=13,
                options=[ft.dropdown.Option("staff"), ft.dropdown.Option("admin")],
            )
            err = ft.Text("", size=11, color="#DC2626")

            def submit(ev):
                if not (name_f.value or "").strip() or not (email_f.value or "").strip() or not (pw_f.value or "").strip():
                    err.value = "Name, email and password are required."
                    page.update()
                    return
                if len((pw_f.value or "").strip()) < 6:
                    err.value = "Password must be at least 6 characters."
                    page.update()
                    return
                try:
                    res = admin_post(f"/admin/staff", json={
                        "name": name_f.value.strip(),
                        "email": email_f.value.strip(),
                        "password": pw_f.value.strip(),
                        "phone": (phone_f.value or "").strip(),
                        "role": role_f.value or "staff",
                    }, timeout=25)
                    if res.status_code in (200, 201):
                        dlg.open = False
                        show_toast("Staff account created!")
                        fetch_staff()
                        render_staff()
                        page.update()
                    else:
            ...
            dlg = ft.AlertDialog(
                title=ft.Text("Add Staff Account", weight=ft.FontWeight.BOLD),
                content=ft.Column([name_f, email_f, pw_f, phone_f, role_f, err], spacing=10, tight=True, width=300),
                actions=[
                    ft.TextButton("Cancel", on_click=lambda ev: (setattr(dlg, "open", False), page.update())),
                    ft.ElevatedButton("Create Account", bgcolor="#DC2626", color="white", on_click=submit),
                ],
            )
            page.dialog = dlg
            dlg.open = True
        ...
        staff_section = ft.Container(
            ...
            content=ft.Column([
                ft.Row([
                    ft.Text("Staff & Seller Accounts", size=13, weight=ft.FontWeight.BOLD, color="#121212"),
                    ft.Container(expand=True),
                    ft.ElevatedButton(
                        "Add Staff", icon=ft.icons.PERSON_ADD, bgcolor="#DC2626", color="white",
                        height=32, style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8)),
                        on_click=open_add_staff_dialog,
                    ),
                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                ft.Text("Create an account for each seller. They sign in to the POS with email + password.", size=10, color="#6B7280"),
    ...
    identifier_field = ft.TextField(
        ...
        label_style=ft.TextStyle(color="#6B7280", size=12),
        color="#111827",
        width=260,
        text_size=14,
        border_color="#DC2626",
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                try:
                    ...
                    pass
                show_toast(detail, is_error=True)
        except httpx.TimeoutException:
            show_toast("Server is waking up. Please try again.", is_error=True)
        except Exception as err:
            show_toast(f"Connection error: {err}", is_error=True)

    password_field.on_submit = handle_email_login
    ...
    def handle_pin_login(e=None):
        ...
        show_toast("Connecting to cloud server...")
    ...
    # Build a simple login view used on startup
    login_view = ft.Container(
        ...
        content=ft.Container(
            ...
            border=ft.border.all(1, "#E5E7EB"),
            content=ft.Column([
                ft.Icon(ft.icons.LOCK_OUTLINED, color="#DC2626", size=44),
                ft.Text("AyuTech Terminal Login", size=18, weight=ft.FontWeight.BOLD, color="#111827"),
                ft.Text("Sign in with your staff account", size=11, color="#6B7280"),
                email_field,
                password_field,
                ft.ElevatedButton(
                    "Sign In",
                    ...
                    width=260,
                    height=42,
                    on_click=handle_email_login,
                ),
                ft.Row([
                    ft.Container(expand=True, content=ft.Divider(color="#E5E7EB")),
                    ft.Text("or", size=11, color="#6B7280"),
                    ft.Container(expand=True, content=ft.Divider(color="#E5E7EB")),
                ], spacing=8, width=260),
                identifier_field,
                pin_field,
                ft.TextButton(
                    "Unlock with legacy PIN",
